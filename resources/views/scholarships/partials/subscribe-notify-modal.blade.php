{{--
    M12 · Subscribe / Notify Modal
    File  : resources/views/scholarships/partials/subscribe-notify-modal.blade.php
    Usage : @include('scholarships.partials.subscribe-notify-modal')
    Trigger in parent view (show.blade.php, only when status = 'coming_soon'):
        <button type="button" onclick="openM12Modal()">Get Notified</button>
    Variables from controller:
        $scholarship   → Scholarship model (->id, ->name, ->opens_at)
        $authUserEmail → string|null  auth()->user()?->email
--}}
...
<div class="sl-overlay" id="m12Overlay" role="dialog" aria-modal="true" aria-labelledby="m12ModalTitle">
    <div class="sl-modal sl-modal--sm">

        {{-- Step 1: Form --}}
        <div id="m12StepForm">
            <div class="sl-modal-header sl-modal-header--teal">
                <div>
                    <h2 class="sl-modal-title sl-modal-title--white" id="m12ModalTitle">Get Notified</h2>
                    <p class="sl-modal-subtitle sl-modal-subtitle--teal-light">
                        {{ $scholarship->name }}
                        &mdash;
                        Opening {{ \Carbon\Carbon::parse($scholarship->opens_at)->format('M Y') }}
                    </p>
                </div>
                <button type="button" class="sl-modal-close sl-modal-close--light" onclick="closeM12Modal()" aria-label="Close">
            ...
            <div class="sl-modal-body">
                <p class="sl-modal-lead" style="margin-bottom: 18px;">
                    We'll email you when this scholarship opens for applications. No spam, just one notification.
                </p>
                <form method="POST" id="m12Form"
                    action="{{ route('scholarships.subscribe', $scholarship->id) }}"
                    onsubmit="m12HandleSubmit(event)">
                    @csrf
                    <div class="sl-form-group">
                        <label class="sl-form-label" for="m12Email">Email Address</label>
                        <input type="email" id="m12Email" name="email" class="sl-form-input"
                            value="{{ $authUserEmail ?? '' }}" placeholder="your@email.com" required>
                    </div>
                    <div class="sl-checkbox-row">
                        <input type="checkbox" id="m12Sms" name="notify_sms" value="1" checked>
                        <label for="m12Sms">Also notify me via SMS</label>
                    </div>
                </form>
            </div>

            <div class="sl-modal-footer">
                <button type="button" class="sl-btn sl-btn--ghost" onclick="closeM12Modal()">Cancel</button>
                <button type="submit" form="m12Form" class="sl-btn sl-btn--primary">&#128276; Notify Me</button>
            </div>
        </div>

        {{-- Step 2: Success --}}
        <div id="m12StepSuccess" class="sl-step-hidden">
            <div class="sl-modal-body sl-modal-body--centered">
                <div class="sl-success-emoji">&#127881;</div>
                <h2 class="sl-modal-title" style="text-align:center;">You're on the list!</h2>
                <p class="sl-modal-lead" style="text-align:center;">
                    We'll notify you at <strong id="m12ConfirmedEmail"></strong>
                    as soon as <strong>{{ $scholarship->name }}</strong> opens for applications.
                </p>
                <a href="{{ route('scholarships.index') }}" class="sl-btn sl-btn--primary sl-btn--full" style="margin-top:4px;">
...
@push('scripts')
<script>
    function openM12Modal() {
        document.getElementById('m12StepForm').classList.remove('sl-step-hidden');
        document.getElementById('m12StepSuccess').classList.add('sl-step-hidden');
        document.getElementById('m12Overlay').classList.add('is-open');
        document.body.style.overflow = 'hidden';
    }
    function closeM12Modal() {
        document.getElementById('m12Overlay').classList.remove('is-open');
        document.body.style.overflow = '';
    }
    function m12HandleSubmit(e) {
        e.preventDefault();
        var email = document.getElementById('m12Email').value.trim();
        if (!email) return;
        document.getElementById('m12ConfirmedEmail').textContent = email;
        fetch(e.target.action, {
            method: 'POST',
            body: new FormData(e.target),
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        document.getElementById('m12StepForm').classList.add('sl-step-hidden');
        document.getElementById('m12StepSuccess').classList.remove('sl-step-hidden');
    }
    document.getElementById('m12Overlay').addEventListener('click', function(e) {
        if (e.target === this) closeM12Modal();
    });
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') closeM12Modal();
    });
</script>
@endpush