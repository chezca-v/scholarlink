{{--
    M3 · Extend Application Deadline Modal
    File  : resources/views/admin/scholarships/partials/extend-deadline-modal.blade.php
    Usage : @include('admin.scholarships.partials.extend-deadline-modal')
    Trigger per row in parent view:
        <button type="button"
            onclick="openM3Modal({{ $scholarship->id }}, '{{ addslashes($scholarship->name) }}', '{{ $scholarship->deadline }}')">
            Extend Deadline
        </button>
...
<div class="sl-overlay" id="m3Overlay" role="dialog" aria-modal="true" aria-labelledby="m3ModalTitle">
    <div class="sl-modal sl-modal--md">

        <div class="sl-modal-header sl-modal-header--center">
            <h2 class="sl-modal-title" id="m3ModalTitle">Extend Application Deadline</h2>
        </div>

        <div class="sl-modal-body">
            <div class="sl-meta-row">
                <span class="sl-meta-label">Scholarship:</span>
                <strong id="m3ScholarshipName"></strong>
            </div>
            <div class="sl-meta-row" style="margin-bottom: 14px;">
                <span class="sl-meta-label">Current Deadline:</span>
                <span class="sl-deadline-badge" id="m3CurrentDeadline"></span>
            </div>

            <hr class="sl-divider" style="margin-bottom: 20px;">

            <form method="POST" id="m3Form" action="">
                @csrf
                @method('PATCH')

                <div class="sl-form-group">
                    <label class="sl-form-label" for="m3NewDeadline">New Deadline</label>
                    <div class="sl-input-icon-wrap">
                        <input type="date" id="m3NewDeadline" name="new_deadline" class="sl-form-input" required>
                ...
                <div class="sl-form-group">
                    <label class="sl-form-label" for="m3Reason">Reason for extension</label>
                    <textarea id="m3Reason" name="reason" class="sl-form-textarea"
                        placeholder="e.g. Extended due to public holiday…"></textarea>
                </div>

                <label class="sl-toggle-label" for="m3NotifyToggle">
                    <span class="sl-toggle-wrap">
                        <input type="checkbox" id="m3NotifyToggle" name="notify_applicants"
                            value="1" class="sl-toggle-input" checked>
        ...
        <div class="sl-modal-footer">
            <button type="button" class="sl-btn sl-btn--ghost" onclick="closeM3Modal()">Cancel</button>
            <button type="submit" form="m3Form" class="sl-btn sl-btn--primary">Save Extension</button>
        </div>
...
@push('scripts')
<script>
    function openM3Modal(scholarshipId, scholarshipName, currentDeadline) {
        document.getElementById('m3ScholarshipName').textContent = scholarshipName;
        document.getElementById('m3CurrentDeadline').textContent = new Date(currentDeadline)
            .toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
        document.getElementById('m3Form').action = '{{ url("admin/scholarships") }}/' + scholarshipId + '/extend-deadline';
        document.getElementById('m3NewDeadline').value = '';
        document.getElementById('m3Reason').value = '';
        document.getElementById('m3NotifyToggle').checked = true;
        document.getElementById('m3Overlay').classList.add('is-open');
        document.body.style.overflow = 'hidden';
    }
    function closeM3Modal() {
        document.getElementById('m3Overlay').classList.remove('is-open');
        document.body.style.overflow = '';
    }
    document.getElementById('m3Overlay').addEventListener('click', function(e) {
        if (e.target === this) closeM3Modal();
    });
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') closeM3Modal();
    });
</script>
@endpush