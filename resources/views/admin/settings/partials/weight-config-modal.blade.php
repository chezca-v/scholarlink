{{--
    M9 · Configure Scoring Weights Modal
    File  : resources/views/admin/settings/partials/weight-config-modal.blade.php
    Usage : @include('admin.settings.partials.weight-config-modal')
    Trigger per row in parent view:
        <button type="button"
            onclick="openM9Modal({{ $scholarship->id }}, '{{ addslashes($scholarship->name) }}', {{ $scholarship->weight_gpa }})">
            Configure Weights
        </button>
    Variables from controller:
        passed inline via onclick — no extra controller variables needed
--}}
...
<div class="sl-overlay" id="m9Overlay" role="dialog" aria-modal="true" aria-labelledby="m9ModalTitle">
    <div class="sl-modal sl-modal--md">

        <div class="sl-modal-header">
            <div>
                <h2 class="sl-modal-title" id="m9ModalTitle">Configure Scoring Weights</h2>
                <p class="sl-modal-subtitle sl-modal-subtitle--teal" id="m9ScholarshipName"></p>
            </div>
            <button type="button" class="sl-modal-close" onclick="closeM9Modal()" aria-label="Close">
        ...
        <hr class="sl-divider" style="margin-top: 20px;">

        <div class="sl-modal-body">
            <div class="sl-info-box sl-info-box--warning">
                <span class="sl-info-box__icon">&#9878;&#65039;</span>
                <p>Both weights must total <strong>100%</strong>. The formula determines how each applicant's final score is computed during blind screening.</p>
            </div>

            <form method="POST" id="m9Form" action="">
                @csrf
                @method('PATCH')

                <div class="sl-weight-row">
                    <span class="sl-weight-label">&#128218; Academic Score (GPA/QPI)</span>
                    <span class="sl-weight-pct" id="m9GpaPct">60%</span>
                </div>
                <input type="range" id="m9GpaSlider" name="weight_gpa"
                    class="sl-weight-slider" min="0" max="100" step="5" value="60"
                    oninput="m9Sync(this.value)">

                <div class="sl-weight-row" style="margin-top: 14px;">
                    <span class="sl-weight-label">&#128176; Financial Need (Income)</span>
                    <span class="sl-weight-pct" id="m9IncPct">40%</span>
                </div>
                <input type="range" id="m9IncSlider"
                    class="sl-weight-slider sl-weight-slider--income" min="0" max="100" step="5" value="40"
                    oninput="m9Sync(100 - parseInt(this.value))">

                <div class="sl-weight-total">
                    <span class="sl-weight-total-label">Total:</span>
                    <span class="sl-weight-total-value">100%</span>
                    <span class="sl-weight-valid" id="m9ValidLabel">&#10003; Valid</span>
                </div>

                <div class="sl-formula-box">
                    <p class="sl-formula-box__formula" id="m9Formula">
                        Final Score = (GPA Score &times; 60%) + (Income Score &times; 40%)
                    </p>
                    <p class="sl-formula-box__sub">Used for blind applicant ranking during evaluation</p>
                </div>
            </form>
        </div>

        <hr class="sl-divider" style="margin: 0;">

        <div class="sl-modal-footer">
            <button type="button" class="sl-btn sl-btn--ghost" onclick="closeM9Modal()">Cancel</button>
            <button type="submit" form="m9Form" id="m9SaveBtn" class="sl-btn sl-btn--primary">&#128190; Save Weights</button>
        </div>
...
@push('scripts')
<script>
    function openM9Modal(scholarshipId, scholarshipName, weightGpa) {
        document.getElementById('m9ScholarshipName').textContent = scholarshipName;
        document.getElementById('m9Form').action = '{{ url("admin/scholarships") }}/' + scholarshipId + '/weights';
        m9Sync(weightGpa);
        document.getElementById('m9Overlay').classList.add('is-open');
        document.body.style.overflow = 'hidden';
    }
    function closeM9Modal() {
        document.getElementById('m9Overlay').classList.remove('is-open');
        document.body.style.overflow = '';
    }
    function m9Sync(gpa) {
        gpa = parseInt(gpa);
        var inc = 100 - gpa;
        document.getElementById('m9GpaPct').textContent  = gpa + '%';
        document.getElementById('m9IncPct').textContent  = inc + '%';
        document.getElementById('m9GpaSlider').value     = gpa;
        document.getElementById('m9IncSlider').value     = inc;
        document.getElementById('m9Formula').textContent =
            'Final Score = (GPA Score \u00D7 ' + gpa + '%) + (Income Score \u00D7 ' + inc + '%)';
        document.getElementById('m9GpaSlider').style.background =
            'linear-gradient(to right,#0F4C5C ' + gpa + '%,#E2E8F0 ' + gpa + '%)';
        document.getElementById('m9IncSlider').style.background =
            'linear-gradient(to right,#C9A84C ' + inc + '%,#E2E8F0 ' + inc + '%)';
    }
    document.getElementById('m9Overlay').addEventListener('click', function(e) {
        if (e.target === this) closeM9Modal();
    });
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') closeM9Modal();
    });
</script>
@endpush