...
@push('styles')
<style>
    .dashboard-main-area { display: grid; grid-template-columns: minmax(0, 1fr) 340px; gap: 20px; align-items: start; }
    .btn-pri { background: linear-gradient(135deg, var(--primary), var(--primary-light)); color: var(--accent-light); border: none; padding: 10px 20px; border-radius: 50px; font-weight: 700; cursor: pointer; }
    @media (max-width: 1240px) { .dashboard-main-area { grid-template-columns: 1fr; } aside { width: 100%; } }
    @media (max-width: 680px) { .dashboard-body { padding: 20px 14px; } }
    .alert-row {
            display: flex;
            gap: 12px;
            padding: 14px;
...
<main class="dashboard-body">
    <div class="dashboard-heading">
        <div>
            @php $organization = auth()->user()->organization ?? null; @endphp
            <p style="font-size:11px; font-weight:700; text-transform:uppercase; color:var(--primary-hover);">{{ $organization?->name ?? 'ScholarLink' }}</p>
            <h2 style="font-family:'Fraunces'; font-size:28px; font-weight:700;">Admin Dashboard</h2>
            <p style="font-size:12px; color:var(--slate); margin-top:2px;">{{ $now->format('l, F j, Y') }} · Academic Year {{ $now->year }}–{{ $now->copy()->addYear()->year }}</p>
        </div>
        <div class="heading-actions">
            <form method="GET" action="{{ route('admin.analytics.export') }}" style="display:inline;">
                <button type="submit" class="btn-pri" style="background:white; border:1px solid var(--border); color:var(--primary);">Export Report</button>
            </form>
    ...
    <div class="dashboard-main-area">
        ...
        <aside>
            @php $quickActions = $quickActions ?? []; @endphp
            <x-quick-actions :actions="$quickActions" />

            @php $breakdownItems = $breakdownItems ?? []; @endphp
...
<script>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('sessionTracker', () => ({
            ...
            logout() { document.getElementById('auto-logout-form').submit(); },
            stayLoggedIn() { this.resetIdleTime(); }
        }));
    });
</script>
@endpush
