...
<head>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .page-frame{background:#FFFFFF;border:1px solid #DFF0EE;border-radius:20px;overflow:hidden;box-shadow:0 8px 24px rgba(15,76,92,0.10);max-width:900px;margin:0 auto;}
        .auth-split{display:grid;grid-template-columns:1fr 1fr;min-height:540px;}
        .auth-left{background:linear-gradient(160deg,#0F4C5C,#1A6B7A);padding:48px 40px;display:flex;flex-direction:column;justify-content:center;position:relative;}
        ...
        .auth-link{font-size:12px;font-weight:600;color:#0F4C5C;cursor:pointer;text-decoration:underline;}
        .form-group{display:flex;flex-direction:column;gap:5px;margin-bottom:14px;}
        .form-label{font-size:12px;font-weight:600;color:#0A3040;}
        .input{font-size:13px;color:#0A3040;border:1.5px solid #DFF0EE;border-radius:10px;padding:9px 13px;width:100%;}
        .btn{font-weight:700;cursor:pointer;border-radius:10px;font-size:13px;padding:9px 20px;border:none;width:100%;}
...
<div class="page-frame">
    <div class="auth-split">
        ...
        <div class="auth-right">
            ...
            <div class="auth-sub">Enter your PLM email address to recover your account.</div>
            
            @if (session('status'))
                <div style="color:green; font-size:12px; margin-bottom:10px;">{{ session('status') }}</div>
            @endif
