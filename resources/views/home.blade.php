...
<head>
    ...
    <style>
    :root {
        --teal-950: #071820; --teal-900: #0F4C5C; --teal-800: #1A6B7A;
        --teal-200: #C8E8E4; --teal-100: #EAF4F3; --teal-50: #F0FAFA;
        --gold: #E8A838; --gold-bg: #FDF4E3;
        --white: #ffffff; --dark: #0A3040; --body: #4A7A80; --muted: #7AACAA; --border: #EAF4F3;
    }
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* NAV */
    nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; background: rgba(255,255,255,0.92); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border-bottom: 1px solid var(--border); transition: box-shadow 0.3s; }
    ...
    .logo { font-family: 'Fraunces', serif; font-size: 20px; font-weight: 700; color: var(--teal-900); letter-spacing: -0.5px; display: flex; align-items: center; gap: 10px; }
    .logo-mark { width: 32px; height: 32px; background: linear-gradient(135deg, var(--teal-900), var(--teal-800)); border-radius: 9px; display: flex; align-items: center; justify-content: center; color: var(--gold); font-size: 15px; }
    ...
    .btn-text { font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 600; padding: 8px 18px; border: none; background: transparent; color: var(--teal-900); cursor: pointer; border-radius: 8px; transition: background 0.2s; }
    .btn-text:hover { background: var(--teal-50); }
    .btn-pill { font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 700; padding: 9px 22px; border: none; background: var(--teal-900); color: var(--gold); border-radius: 999px; cursor: pointer; box-shadow: 0 4px 14px rgba(15,76,92,0.22); transition: all 0.25s; }
    .btn-pill:hover { background: var(--teal-800); transform: translateY(-2px); }

    /* HERO */
    .hero { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 120px 48px 80px; text-align: center; background: var(--white); position: relative; overflow: hidden; }
    .hero-bg { position: absolute; top: 0; left: 0; right: 0; height: 640px; background: radial-gradient(ellipse 80% 60% at 50% -10%, #E8F8F5 0%, transparent 65%); pointer-events: none; }
    .fi { position: absolute; opacity: 0.13; pointer-events: none; animation: floatUD 8s ease-in-out infinite; z-index: 0; }
    ...
    @keyframes floatUD { 0%,100%{ transform: translateY(0) rotate(0deg); } 50%{ transform: translateY(-28px) rotate(4deg); } }

    .hero-badge { display: inline-flex; align-items: center; gap: 8px; background: var(--teal-50); border: 1px solid var(--teal-200); border-radius: 999px; padding: 6px 16px; font-size: 12px; font-weight: 600; color: var(--body); margin-bottom: 32px; position: relative; z-index: 1; animation: fadeDown 0.6s ease both; }
    ...
    .hero-title { font-family: 'Fraunces', serif; font-size: clamp(52px, 6vw, 90px); font-weight: 900; letter-spacing: -3px; line-height: 0.95; color: var(--dark); max-width: 820px; margin-bottom: 28px; position: relative; z-index: 1; min-height: 2em; animation: fadeUp 0.7s 0.1s ease both; }
    .hero-sub { font-size: 17px; color: var(--muted); line-height: 1.75; max-width: 520px; margin: 0 auto 40px; position: relative; z-index: 1; animation: fadeUp 0.7s 0.2s ease both; }
    .hero-actions { display: flex; align-items: center; gap: 16px; justify-content: center; margin-bottom: 64px; position: relative; z-index: 1; animation: fadeUp 0.7s 0.3s ease both; }
    .btn-hero-main { font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 700; padding: 14px 32px; border: none; background: var(--teal-900); color: var(--gold); border-radius: 12px; cursor: pointer; box-shadow: 0 8px 24px rgba(15,76,92,0.24); transition: all 0.25s; }
    .btn-hero-main:hover { transform: translateY(-3px); box-shadow: 0 14px 36px rgba(15,76,92,0.32); background: var(--teal-800); }
    .btn-hero-ghost { font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 600; color: var(--body); background: transparent; border: none; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: color 0.2s, gap 0.2s; }
    .btn-hero-ghost:hover { color: var(--teal-900); gap: 12px; }

    .hero-social-proof { display: flex; align-items: center; gap: 16px; position: relative; z-index: 1; animation: fadeUp 0.7s 0.4s ease both; }
    .avatars { display: flex; }
    .avatar { width: 32px; height: 32px; border-radius: 50%; border: 2px solid var(--white); background: var(--teal-200); color: var(--teal-900); font-size: 12px; font-weight: 700; display: flex; align-items: center; justify-content: center; margin-left: -8px; }
    .avatar:first-child { margin-left: 0; }
    .proof-text { font-size: 13px; color: var(--muted); }
    .proof-text strong { color: var(--teal-900); }

    /* HERO CARDS */
    .hero-visual { max-width: 900px; width: 100%; margin: 64px auto 0; position: relative; z-index: 1; animation: fadeUp 0.8s 0.5s ease both; }
    .cards-row { display: grid; grid-template-columns: repeat(3,1fr); gap: 16px; }
    .mini-card { background: var(--white); border: 1px solid var(--border); border-radius: 16px; padding: 20px; text-align: left; box-shadow: 0 4px 16px rgba(15,76,92,0.06); transition: all 0.3s; }
    ...
    .mc-badge.closed  { color: #6B7280; background: #F3F4F6; }
    .mc-title { font-family: 'Fraunces', serif; font-size: 14px; font-weight: 700; color: var(--dark); margin-bottom: 6px; line-height: 1.3; }
    .mc-snippet { font-size: 11px; color: var(--muted); margin-bottom: 10px; }
    .mc-bar { height: 3px; background: var(--border); border-radius: 999px; overflow: hidden; }
    ...
    .logos-row { display: flex; gap: 40px; align-items: center; flex: 1; flex-wrap: wrap; }
    .logo-item { font-family: 'Fraunces', serif; font-size: 14px; font-weight: 700; color: var(--teal-200); letter-spacing: -0.5px; transition: color 0.2s; }
    ...
    .step-num { width: 44px; height: 44px; border-radius: 50%; background: var(--teal-50); border: 1px solid var(--teal-200); display: flex; align-items: center; justify-content: center; font-family: 'Fraunces', serif; font-size: 17px; font-weight: 700; color: var(--teal-900); }
    ...
    .filter { font-size: 12px; font-weight: 600; padding: 7px 16px; border-radius: 999px; border: 1px solid var(--teal-200); background: var(--white); color: var(--body); cursor: pointer; transition: all 0.2s; }
    ...
    .s-card { background: var(--white); border: 1px solid var(--border); border-radius: 18px; padding: 24px; display: flex; flex-direction: column; cursor: pointer; transition: all 0.3s; color: inherit; }
    ...
    .sc-org { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--muted); }
    ...
    .sc-title { font-family: 'Fraunces', serif; font-size: 17px; font-weight: 700; color: var(--dark); margin-bottom: 4px; line-height: 1.3; }
    .sc-tagline { font-size: 12px; color: var(--muted); font-style: italic; margin-bottom: 12px; line-height: 1.4; }
    .sc-snippets { display: flex; flex-direction: column; gap: 5px; margin-bottom: 16px; }
    .sc-snippet { font-size: 12px; color: var(--body); display: flex; align-items: center; gap: 8px; }
    .sc-dot { width: 3px; height: 3px; border-radius: 50%; background: var(--teal-200); flex-shrink: 0; }
    .sc-footer { margin-top: auto; padding-top: 14px; border-top: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; }
    .sc-deadline { font-size: 11px; color: var(--muted); }
    .sc-income { font-size: 11px; font-weight: 600; color: var(--teal-900); }
    ...
    .feat-card { background: var(--teal-50); border: 1px solid var(--border); border-radius: 20px; padding: 32px; transition: all 0.3s; }
    .feat-card:hover { border-color: var(--teal-200); box-shadow: 0 8px 24px rgba(15,76,92,0.07); }
    .feat-card.highlighted { background: var(--teal-900); border-color: transparent; grid-column: span 2; display: grid; grid-template-columns: 1fr 1fr; gap: 48px; align-items: center; }
    .feat-icon { width: 44px; height: 44px; background: var(--white); border: 1px solid var(--teal-200); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 20px; margin-bottom: 16px; }
    ...
    .cta-badge { display: inline-flex; align-items: center; gap: 8px; background: var(--gold-bg); border: 1px solid rgba(232,168,56,0.28); border-radius: 999px; padding: 6px 16px; font-size: 12px; font-weight: 600; color: var(--gold); margin-bottom: 28px; }
    ...
    .btn-cta-main { font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 700; padding: 14px 32px; border: none; background: var(--teal-900); color: var(--gold); border-radius: 12px; cursor: pointer; box-shadow: 0 8px 24px rgba(15,76,92,0.22); transition: all 0.25s; }
    .btn-cta-main:hover { transform: translateY(-2px); }
    .btn-cta-sec { font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 600; padding: 14px 32px; border: 1px solid var(--teal-200); background: transparent; color: var(--body); border-radius: 12px; cursor: pointer; transition: all 0.2s; }
    ...
    .footer-links li a { font-size: 13px; color: rgba(255,255,255,0.42); transition: color 0.2s; }
    ...
    @media (max-width: 768px) {
        ...
        .how, .scholarships, .features, .cta { padding: 72px 24px; }
    }
    </style>
</head>
<body>
...
<section class="hero" id="home">
    ...
    <div class="fi" style="top:15%;left:8%;font-size:52px;">🎓</div>
    ...
    <h1 class="hero-title"><span id="typing-target"></span></h1>
    ...
    <div class="hero-social-proof">
        <div class="avatars">
            <div class="avatar">J</div><div class="avatar">M</div>
            <div class="avatar">A</div><div class="avatar">R</div><div class="avatar">+</div>
        </div>
        <p class="proof-text">
            Joined by <strong>{{ number_format($totalApplicants) }}+ students</strong> across the Philippines
        </p>
    </div>

    {{-- 3 featured scholarships from DB --}}
    <div class="hero-visual">
        <div class="cards-row">
            @foreach ($featuredScholarships as $scholarship)
            <div class="mini-card">
                <div class="mc-top">
                    <span class="mc-org">{{ $scholarship->provider_name }}</span>
                    <span class="mc-badge {{ $scholarship->status }}">{{ ucfirst($scholarship->status) }}</span>
                </div>
                <div class="mc-title">{{ $scholarship->name }}</div>
                <div class="mc-snippet">{{ $scholarship->benefit_snippet_1 }}</div>
    ...
    <div class="scholarships-inner">
        ...
        <div class="s-grid">
            ...
            <a href="{{ route('scholarships.show', $scholarship->id) }}"
               class="s-card reveal d{{ ($loop->index % 3) + 1 }}"
               data-status="{{ $scholarship->status }}">

                <div class="sc-top">
                    <span class="sc-org">{{ $scholarship->provider_name }}</span>
                    <span class="sc-status {{ $scholarship->status }}">{{ ucfirst($scholarship->status) }}</span>
                </div>

                <div class="sc-title">{{ $scholarship->name }}</div>
                <div class="sc-tagline">{{ $scholarship->tagline }}</div>

                <div class="sc-snippets">
                    @if ($scholarship->benefit_snippet_1)
                        <div class="sc-snippet"><span class="sc-dot"></span>{{ $scholarship->benefit_snippet_1 }}</div>
                    @endif
                    @if ($scholarship->benefit_snippet_2)
                        <div class="sc-snippet"><span class="sc-dot"></span>{{ $scholarship->benefit_snippet_2 }}</div>
                    @endif
                    @if ($scholarship->slots > 0)
                        <div class="sc-snippet"><span class="sc-dot"></span>{{ $scholarship->slots }} available slots</div>
                    @endif
                </div>

                <div class="sc-footer">
                    <span class="sc-deadline">
                        Deadline: {{ \Carbon\Carbon::parse($scholarship->deadline)->format('M d, Y') }}
                    </span>
                    @if ($scholarship->income_bracket)
                        <span class="sc-income">{{ $scholarship->income_bracket }}</span>
                    @endif
                </div>
            </a>
            @endforeach
...
<section class="features" id="features">
    <div class="features-inner">
        ...
        <div class="feat-grid">
            ...
            <div class="feat-card reveal d1">
                <div class="feat-icon">📡</div>
                <div class="feat-title">SMS Hardware Gateway</div>
                <div class="feat-desc">Critical updates via SMS even without internet — powered by ESP32 + GSM. No student left behind.</div>
            </div>
            <div class="feat-card reveal d2">
                <div class="feat-icon">🗂️</div>
...
<section class="cta" id="about">
    <div class="cta-inner">
        ...
        <div class="stats-row reveal">
            <div class="stat">
                <div class="stat-num">{{ $totalScholarships }}</div>
            ...
            <div class="stat">
                <div class="stat-num">{{ $openScholarships }}</div>
...
(function () {
    ...
    // Typewriter
    const phrases = ["One Profile.\nEvery Scholarship.", "Your Future\nStarts Here.", "AI-Matched.\nPH-Built."];
    const target = document.getElementById('typing-target');
    let pi = 0, ci = 0, del = false;
    function type() {
        target.innerHTML = phrases[pi].substring(0, ci).replace(/\n/g, '<br>');
        if (!del && ci < phrases[pi].length)       { ci++; setTimeout(type, 80); }
        else if (!del && ci === phrases[pi].length) { setTimeout(() => { del = true; type(); }, 3200); }
        else if (del && ci > 0)                    { ci--; setTimeout(type, 40); }
        else { del = false; pi = (pi + 1) % phrases.length; setTimeout(type, 400); }
    }
    type();
})();
</script>
