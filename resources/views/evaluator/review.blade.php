...
@section('page_subtitle', '/evaluator/review/' . ($application->reference_code ?? 'A-'.$application->id))
      ...
      <div class="card">
        ...
        <div class="score-row">
          ...
          <div class="score-slider-wrap">
            <input type="range" name="gpa_score" id="gpa_score" min="0" max="100" value="{{ old('gpa_score', $evaluation->gpa_score ?? 0) }}" oninput="document.getElementById('gpa_val').textContent=this.value; calculateScore();">
          </div>
          <div class="score-val" id="gpa_val">{{ old('gpa_score', $evaluation->gpa_score ?? 0) }}</div>
        </div>
        <div class="score-row">
          <div class="score-label">Financial Need <small>Weight: {{ $application->scholarship->weight_income ?? 40 }}%</small></div>
          <div class="score-slider-wrap">
            <input type="range" name="income_score" id="income_score" min="0" max="100" value="{{ old('income_score', $evaluation->income_score ?? 0) }}" oninput="document.getElementById('income_val').textContent=this.value; calculateScore();">
          </div>
          <div class="score-val" id="income_val">{{ old('income_score', $evaluation->income_score ?? 0) }}</div>
        </div>
