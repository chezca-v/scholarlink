...
@section('topnav_title', 'Notifications')

@section('content')
<div class="breadcrumb">
    <span>Home</span><span class="sep">/</span><span class="current">Notifications</span>
</div>
...
<div class="card">
    <div class="section-header" style="margin-bottom: 24px;">
        <div>
            ...
            <small>System alerts and general notifications</small>
        </div>
    ...
    @else
        <div style="display: flex; flex-direction: column; gap: 12px;">
            @foreach($notifications as $notif)
                <div style="display: flex; gap: 16px; padding: 16px; border: 1.5px solid {{ $notif->is_read ? 'var(--border)' : 'var(--primary-light)' }}; border-radius: 12px; background: {{ $notif->is_read ? 'var(--surface)' : 'var(--page-bg)' }}; position: relative;">
                    @if(!$notif->is_read)
                    ...
                    <div style="width: 42px; height: 42px; border-radius: 10px; background: rgba(15,76,92,0.1); color: var(--primary); display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0;">
                        @if(str_contains(strtolower($notif->title), 'fraud') || str_contains(strtolower($notif->title), 'alert'))
                            ⚠️
                        @elseif(str_contains(strtolower($notif->title), 'error') || str_contains(strtolower($notif->title), 'fail'))
                            ✖️
                        @elseif(str_contains(strtolower($notif->title), 'system'))
                            ⚙️
                        @else
                            🔔
                        @endif
                    ...
                    @if(!$notif->is_read)
                        <div style="display: flex; align-items: center;">
                            <form action="{{ route('superadmin.notifications.markRead', $notif->id) }}" method="POST">
                                @csrf
