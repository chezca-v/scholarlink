{{--
    M8 · Rejection Feedback Modal
    File  : resources/views/applicant/applications/partials/rejection-feedback-modal.blade.php
    Usage : @include('applicant.applications.partials.rejection-feedback-modal')
    Trigger in parent view (track.blade.php, only when status = 'rejected'):
        <button type="button" onclick="openM8Modal()">View Feedback</button>
    Variables from controller:
        ...
        $suggestedScholarships  → Collection (->name, ->amount, ->match_percent)
--}}
...
<div class="sl-overlay" id="m8Overlay" role="dialog" aria-modal="true" aria-labelledby="m8ModalTitle">
    <div class="sl-modal sl-modal--md sl-modal--scroll">

        <div class="sl-modal-header">
            <div>
                <h2 class="sl-modal-title" id="m8ModalTitle">Application Not Approved</h2>
                <p class="sl-modal-subtitle sl-modal-subtitle--amber">
        ...
        <div class="sl-modal-body">
            ...
            <div class="sl-rejection-box">
                <p class="sl-rejection-box__label">Rejection Reason</p>
                <p class="sl-rejection-box__text">{{ $application->rejection_reason }}</p>
            </div>

            @if($application->evaluator_notes)
                <p class="sl-eyebrow">Evaluator Notes</p>
                <div class="sl-eval-notes-box">{{ $application->evaluator_notes }}</div>
            @endif

            @if($suggestedScholarships->isNotEmpty())
                <p class="sl-eyebrow">Suggested Alternatives</p>
                @foreach($suggestedScholarships as $alt)
                    <div class="sl-alt-item">
                        <div>
                            <p class="sl-alt-name">{{ $alt->name }}</p>
                            <p class="sl-alt-amount">&#8369;{{ number_format($alt->amount) }}/semester</p>
                        </div>
                        <span class="sl-alt-match">{{ $alt->match_percent }}%</span>
                    </div>
                @endforeach
            @endif
        ...
        <div class="sl-modal-footer">
            <button type="button" class="sl-btn sl-btn--ghost" onclick="closeM8Modal()">Close</button>
            <a href="{{ route('scholarships.index') }}" class="sl-btn sl-btn--secondary">Browse all scholarships</a>
        </div>
...
@push('scripts')
<script>
    function openM8Modal() {
        document.getElementById('m8Overlay').classList.add('is-open');
        document.body.style.overflow = 'hidden';
    }
    function closeM8Modal() {
        document.getElementById('m8Overlay').classList.remove('is-open');
        document.body.style.overflow = '';
    }
    document.getElementById('m8Overlay').addEventListener('click', function(e) {
        if (e.target === this) closeM8Modal();
    });
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') closeM8Modal();
    });
</script>
@endpush